...
def setup_model(args):
    ...
    """
    if (args.use_autograd and 'autograd' in args.train_func and 'autograd' in args.val_func) or 'cdep' in args.train_func:
        model = torchvision.models.__dict__[args.model](pretrained=args.pretrained).cuda(args.gpu)
        
        if 'vgg' in args.model:
            if  model.classifier[6].weight.shape[-2] != args.num_classes:
                model.classifier[6] = nn.Linear(model.classifier[6].weight.shape[-1], args.num_classes).cuda(args.gpu)
        else:
